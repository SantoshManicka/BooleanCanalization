...
# ComputeGeneralizedKeff(func, k, ComputeKeff = T, ReturnSymmSchs = T)
...
# BASIC IDEA: 
# Identify "symmetric schemata" on the basis of the number of 1s in the input values. We may think of this as a
# generalization of identifying input schemata where the number of 1s must cover the entire range of possible values
# in the interval [0, D] where D is the dimension of the cube. Symmetric schemata, on the other hand, may comprise input values
# with number of 1s occupying partial sub-intervals of [0, D].

# SYMMETRIC SCHEMATA: 
# Following is a method for identifying symmetric schemata. For each cube and for a given output value (0 or 1), extract the 
...
# of that set. For a set of input values with a given number of 1s to form a symmetric schema, its cardinality must be equal
# to C(D, NumOnes) where D is the dimension of the cube. For example, |(0'1'1')| = C(3, 2) = 3. We only identify symmetric
...
# more than one symmetric schema. For example, (0'0'0'1'1'#') and (1'1'1'1'1'#') are two non-overlapping schemata in a 
# 6-dimensional cube.

# COMPOSITE SCHEMATA:
# A composite schema is a symmetric schema with at least one fixed (non-permuting) '#' and at least one permuting permuting 
# '#'; it is named so because it contains the characteristic features of both an ordinary schema and a two-symbol schema.
# We identify 'composite' schemata by combining two or more symmetric schemata, as identified above, on the basis that
...
# by combining two or more parallel symmetric schemata. We also identify composite schemata that bridge *portions* of two 
...
# Step 2: Identify all ordinary symmetric schemata in each subcube in the set obtained in the previous step and for each 
# output value.
# Step 3: Identify all composite schemata from the set of ordinary symmetric schemata obtained in the previous step and for 
# each output value.
...
# As mentioned above, this program identifies only those symmetric schemata that contain at least one permuting `#'.
...
# of the hypercube exists that region wouldn't have to be combed further, the GeneralizedKeff calculation method here would have to
...
# Note of reassurance: Before coding this version, we had two versions that identified symmetric schemata in an inefficient 
# way -- by enumerating all possible symmetric schemata for a given dimension (number of permuting variables), and then matching 
...
ComputeGeneralizedKeff <- function(Func, K, ComputeKeff = F, ReturnSymmSchs = F, NumCores = 1) {
  ...
  # Given the current set of Essential schemata (EssSch) so far and a new set of SymmSch obtained from a set
  # of parallel cubes (as determined by a set of fixed variables), update the current EssSch set.
  # The basic idea is to compare the set of covered states of each SymmSch in the new set with those
  # of the SymmSch in the current EssSch set. 
  # Note that it's possible that a SymmSch obtained from a lower dimension cube could cover one from a 
  # higher dimension. E.g., (#1'#') covers (1'1'#'); former exists at Dim = 2 while the latter at Dim = 3. 
  # However, a SymmSch at a given Dim, obtained from a particular set of parrallel cubes, can't cover 
  # one another. The variable 'LastSchIdx' is used for this purpose -- it marks the index of the last SymmSch
  # in the current EssSch set, each SymmSch in the new set will be compared with every SymmSch only upto LastSchIdx.
  
  FilterEssentialSchemata <- function(EssSchCvrdStates, LastSchIdx) {
    
    CvrdStsIdx = c()
    
    if (LastSchIdx == 0) return(CvrdStsIdx)
    
    EssSchLen = length(EssSchCvrdStates)
    
    for (i in (LastSchIdx + 1) : EssSchLen) {
      
      CmpCvrdSts = EssSchCvrdStates[[i]]
    
      for (j in 1 : LastSchIdx) {
        
        CvrdSts = EssSchCvrdStates[[j]]
        
        IsCmprFullyCvrd = all(is.element(CmpCvrdSts, CvrdSts))
        
        if (IsCmprFullyCvrd) {
          
          CvrdStsIdx = c(CvrdStsIdx, i)
          
          if (i == EssSchLen) break
          
        } else {
          
          IsFullyCvrd = all(is.element(CvrdSts, CmpCvrdSts))
          
          if (IsFullyCvrd) CvrdStsIdx = c(CvrdStsIdx, j)
	...
	# Identifies the symmetric schemata present in a cube of a given Dim. For Dim = 2, for example, 
	# the possible symmetric schemata are (#'#'), (0'#'), (1'#'). The following are examples that are NOT
	# symmetric schemata and therefore won't be identified by this method: (0'0'), (1'0'), (1#). 
	...
	ScanCube <- function(LUT, Func, fxdvars, fxdvals, Dim, K) {
		...
		for (OutputVal in 0 : 1) {
			...
			if (NumStates == 2^Dim) {  # the entire cube is a single symmetric schema with only permuting '#'
			...
			} else {  # may contain (multiple) symmetric schemata
	...
	ScanParallelCubes <- function(idx, FixedVars, FixedVals, LUT, NodeKred, NodeKredGeneral, Func, Dim, K, 
	  ...
	  # Step 1: Consider a single set of parallel subcubes of a given dimension
		...
		# Step 3: Identify all composite schemata from the set of ordinary symmetric schemata obtained in the previous step 
					...
					for (MinNumOnes in 0 : (NumPermutVars - NumWC)) {
						...
						if (NumMtchngSS > 1 & NumWC < NumPermutVars) {  # form Composite SymmSchs
							...
							for (cbidx in 1 : length(Cubes$Schemata)) {  # this is assuredly non-empty
								...
								# Step 4: Record the dimension of the symmetric schema against every input vector it covers if and only if 
								# the current largest covering symmetric schema's dimension is smaller. 
			...
			if (RetSS) {
				...
				for (scn in 1 : length(ScanResults)) {
				  ...
				  if (!is.null(ResEssSch)) {
				    ...
				    LastSchIdx = length(EssentialSchemata)
				    ...
				    FiltSchIdx = FilterEssentialSchemata(EssSchCvrdStates, LastSchIdx)
				    
				    FiltSchLen = length(FiltSchIdx)
				    
				    if (FiltSchLen > 0) {
				      
				      EssentialSchemata = EssentialSchemata[ -FiltSchIdx ]
				      EssSchCvrdStates = EssSchCvrdStates[ -FiltSchIdx ]
				      EssSchOutputs = EssSchOutputs[ -FiltSchIdx ]
	...
	GeneralizedKeff = K - mean(NodeKredGeneral)
	...
	if (ComputeKeff) {
		...
		} else return(list(Keff = Keff, GeneralizedKeff = GeneralizedKeff))
